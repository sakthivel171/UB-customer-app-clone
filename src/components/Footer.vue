<template>
  <router-link v-if="cart.length > 0" to="/cart"
    class="  flex fixed bottom-16 bg-[#73983c]  text-white px-4  py-2 w-full max-w-[500px] rounded-t-md mx-auto shadow-md justify-between">
    <div class="flex items-center font-roboto text-base gap-2"><ion-icon name="cart"
        class="text-white text-3xl"></ion-icon>Cart</div>
  ...
  <footer class="fixed bottom-0 w-full max-w-[500px] mx-auto">
    <div
      ...
      <router-link v-for="(item, index) in menu" :key="index" :to="item.link" class="flex-1 py-2 "
